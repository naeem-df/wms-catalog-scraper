"""
Alert catalog spider using scrapy-playwright.

Handles authentication and data extraction from Alert auto parts catalog.
"""
...
class AlertSpider(scrapy.Spider):
    """
    Spider for Alert auto parts catalog (TopMotive platform).
    ...
    # URLs
    LOGIN_URL = "https://web1.carparts-cat.com/default.aspx?10=7FD0A9CE5E4D41DEACA81956B6408394494004&14=4&12=1003"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.username = self.settings.get('ALERT_USERNAME', 'MMOIABK')
        self.password = self.settings.get('ALERT_PASSWORD', 'RMVW1')
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        logger.info(f"Starting Alert spider with username: {self.username}")
    ...
    async def parse_login(self, response: Response) -> Generator[Request, None, None]:
        ...
        try:
            ...
            if await self._is_logged_in(page):
                logger.info("Already logged in to Alert catalog")
                self.logged_in = True
            ...
            if not username_field:
                ...
                await page.screenshot(path='/tmp/alert_login_error.png')
                return
            ...
            if await self._is_logged_in(page):
                self.logged_in = True
                logger.info("Successfully logged into Alert catalog")
                await page.screenshot(path='/tmp/alert_login_success.png')
                
                # Navigate to catalog and start scraping
                yield from await self._navigate_to_catalog(page, response)
            else:
                logger.error("Login failed for Alert catalog")
                await page.screenshot(path='/tmp/alert_login_failed.png')
                
        except Exception as e:
            logger.error(f"Error during login: {e}")
            await page.screenshot(path='/tmp/alert_login_exception.png')
        finally:
            # Don't close the page - we need it for subsequent requests
            pass
    ...
    async def _navigate_to_catalog(self, page, response: Response) -> Generator[Request, None, None]:
        ...
        # Get current URL (might be catalog after login)
    ...
    async def parse_categories(self, response: Response) -> Generator[Request, None, None]:
        ...
        # Queue category pages for scraping
        for category in unique_categories[:5]:  # Limit for testing
            yield Request(
                url=category['url'],
                meta={
                    "playwright": True,
    ...
    async def parse_category(self, response: Response) -> Generator[CatalogPartItem, None, None]:
        """Parse products from a category page."""
        page = response.meta.get("playwright_page")
        category_name = response.meta.get("category_name", "Unknown")
        
        if not page:
            logger.error("No Playwright page in parse_category")
            return
        
        logger.info(f"Parsing category: {category_name}")
        
        # Try to extract products directly if no categories found
    ...
    async def _extract_part_from_row(self, page, row, category_name: str):
        ...
        try:
            ...
            if sku or description:
                ...
                item['sku'] = sku or f"ALERT-{hash(description or '') % 1000000:06d}"
                item['supplier'] = 'alert'
