"""
Motus catalog parser.

Handles navigation and data extraction from Motus auto parts catalog.
...
class MotusParser:
    """Parser for Motus auto parts catalog."""
    
    def __init__(self, headless: bool = True):
        """Initialize Motus parser."""
        self.supplier = "motus"
    ...
    async def start(self):
        ...
        self.session = AuthSession("motus", headless=self.headless)
        self.page = await self.session.start()
        
        # Login
        success = await self.session.login()
        if not success:
            raise RuntimeError("Failed to login to Motus catalog")
        
        logger.info("Motus parser started and authenticated")
    ...
    async def scrape(self) -> List[Part]:
        ...
        try:
            ...
            logger.info(f"Found {len(categories)} categories in Motus catalog")
            ...
            logger.info(f"Total parts scraped from Motus: {len(all_parts)}")
            return all_parts
            
        except Exception as e:
            logger.error(f"Motus scraping failed: {e}")
            raise
    ...
    async def close(self):
        ...
        if self.session:
            ...
            logger.info("Motus parser closed")
